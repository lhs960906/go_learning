...
type FileDownloader struct {
	fileSize       int        // 待下载文件的大小
	url            string     // 带下载文件的url
	outputFileName string     // 下载后的本地文件名
	totalPart      int        // 下载线程数量
	outputDir      string     // 文件下载目录
	doneFilePart   []filePart // 文件内容分片(按序存放文件内容的缓冲区)
}
...
func main() {
	startTime := time.Now()
	// 下载文件地址
	var url string = "https://download.jetbrains.com/go/goland-2020.2.2.dmg"
